...
signatures_1(StMap, FuncDb0, State0) ->
    case wl_next(State0#sig_st.wl) of
        ...
        empty ->
            ...
            %% updates.
            #sig_st{updates=Same,committed=Same} = State0,
...
init_sig_st(StMap, FuncDb) ->
    ...
    #sig_st{ committed=#{},
             ...
             wl=wl_defer_list(Roots, #worklist{}) }.
...
type(put_map, _Args, _Anno, _Ts, _Ds) ->
    #t_map{};
type(put_list, [_,_]=Args, _Anno, Ts, _Ds) ->
    [Head, Tail] = raw_types(Args, Ts),
    {ElementType, Proper} = case beam_types:meet(Tail, #t_list{}) of
                                #t_list{type=T,proper=true} ->
                                    {beam_types:join(Head, T), true};
                                #t_cons{head=T,proper=true} ->
                                    {beam_types:join(Head, T), true};
                                nil ->
                                    {Head, true};
                                _ ->
    ...
    #t_cons{head=ElementType,proper=Proper};
